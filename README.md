...
| Paseo              | A build supporting `registerProvider` and `@getpaseo/plugin/provider`. The verified baseline is main commit [`c424f829`](https://github.com/getpaseo/paseo/tree/c424f82922fcd36aa9cc9e473644bca04417b420). |
...
| `npm run build`                                                | Generate `dist/index.server.js` and verify that the development SDK snapshot is excluded.        |
...
`test:upstream` requires a Paseo checkout with dependencies installed and checks that its public API source matches the stored snapshot. It uses a test implementation of the ZCode host.
...
- `vendor/paseo/` — Public Provider API snapshot used only for typechecking and tests.
- `docs/` — Architecture decisions, verification notes, and remaining work.

The development SDK snapshot is excluded from the runtime bundle; Paseo supplies the SDK at runtime. See the [ADRs](docs/adr/README.md) for design rationale and [NOTICE.md](NOTICE.md) for source and icon attribution.
